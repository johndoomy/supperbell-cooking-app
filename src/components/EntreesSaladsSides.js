...
const EntreesSaladsSides = ({ deleteIngredient, familyRecipe, index, updateRecipe, selectedRecipe }) => {
  return (
    <div>
      {JSON.stringify(selectedRecipe.ingredients) !== JSON.stringify(["none"]) &&
        <ul className="listOfIngredients">
          {selectedRecipe.ingredients && selectedRecipe.ingredients.map((ingredient, recipeIndex) => (
            <IngredientItem deleteIngredient={deleteIngredient} key={recipeIndex} selectedRecipe={selectedRecipe} familyRecipe={familyRecipe} ingredient={ingredient} recipeIndex={recipeIndex} />))}
        </ul>
      }
